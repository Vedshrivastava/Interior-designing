          ...
          <div key={date} className='order-date-group'>
            ...
            {groupedOrders[date].map((order, index) => (
              <div key={index} className='order-item'>
                {order.image ? ( // Check if image is available
                  <img src={order.image} alt={order.designName} /> // Use image from API
                ) : (
                  <img src={assets.parcel_icon} alt="" /> // Fallback icon
                )}
                <div>
                  <p className='order-item-name'>{order.name}</p>
                  <p className='order-item-phone'>{order.phoneNumber}</p>
                  <p>{order.email}</p>
                  <p className='order-item-design-name'>{order.designName || 'No design name'}</p> {/* Display design name */}
                </div>
