...
const VendorPicker = ({ url, selectedVendorId, onChange }) => {
    ...
    useEffect(() => {
        axios.get(`${url}/api/finance/vendors/list`, authHeader)
            .then(res => { if (res.data.success) setVendors(res.data.data.filter(NON_CONTRACTOR)); })
            .catch(() => {});
...
/* Vendors here is the same financeVendor data as everywhere else, just
   ...
   computed-on-read shape as the Contractor Ledger. */
const ProcurementPage = ({ url }) => {
    ...
    return (
        ...
        >
            ...
            {activeTab === 'ledger' && (
                <>
                    <VendorPicker url={url} selectedVendorId={selectedVendorId} onChange={setSelectedVendorId} />
                    {selectedVendorId
