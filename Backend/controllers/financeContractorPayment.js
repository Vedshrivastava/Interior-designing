...
const listContractorPayments = async (req, res) => {
    try {
        ...
        const items = await FinanceContractorPayment.find(filter).populate('bankAccountId', 'accountName').populate('tdsSectionId', 'name code').sort({ date: -1, createdAt: -1 }).lean();
...
const uploadAttachment = async (file) => {
    if (!file) return '';
    try {
        const url = await uploadRawFile(file.path, 'contractor_payment_attachments');
...
const addContractorPayment = async (req, res) => {
    try {
        ...
        await logActivity({
            ...
            summary: `Contractor ${vendor.name} paid`,
