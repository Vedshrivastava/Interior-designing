...
const addLabourer = async (req, res) => {
    try {
        const { name, accountName, bankName, accountNumber, ifscCode, notes, labourProviderId, labourProviderRatePerSqft } = req.body;
        ...
        if (labourProviderId) {
            ...
            try {
                await assertLabourProviderVendor(labourProviderId);
            } catch (err) {
        ...
        const item = new FinanceLabourer({
            ...
            labourProviderId: labourProviderId || null,
            labourProviderRatePerSqft: labourProviderId ? Number(labourProviderRatePerSqft) : null,
        });
        await item.save();
...
const updateLabourer = async (req, res) => {
    try {
        const { _id, name, accountName, bankName, accountNumber, ifscCode, notes, labourProviderId, labourProviderRatePerSqft } = req.body;
