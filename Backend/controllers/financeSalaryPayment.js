...
const listSalaryPayments = async (req, res) => {
    try {
        ...
        const items = await FinanceSalaryPayment.find(filter).populate('bankAccountId', 'accountName').populate('tdsSectionId', 'name code').sort({ date: -1, createdAt: -1 });
...
const addSalaryPayment = async (req, res) => {
    try {
        const { employeeId, month, amount, date, paymentMode, bankOrCashLabel, bankAccountId, utrNumber, notes, tdsSectionId, tdsAmount } = req.body;
        ...
        // See financeContractorPayment.js's identical comment — net of TDS
        // is what actually leaves cash; the employee's own salary-ledger
        // balance (financeSalaryLedger.js) stays gross.
        if (!bankAccountId) {
            await FinanceCashEntry.create({
                date, type: 'out', amount: Number(amount) - (resolvedTdsAmount || 0),
...
const updateSalaryPayment = async (req, res) => {
    try {
        const { _id, month, amount, date, paymentMode, bankOrCashLabel, utrNumber, notes, tdsSectionId, tdsAmount } = req.body;
