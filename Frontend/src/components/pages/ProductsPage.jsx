    ...
    <div className="prod-modal-backdrop" onClick={closeModal} role="dialog" aria-modal="true">
      <div className="prod-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="prod-modal-gallery">
          ...
          {images.length > 1 && <div className="prod-modal-thumbs">{images.map((src, i) => <button key={i} className={`prod-modal-thumb${i === activeThumb ? ' active' : ''}`} onClick={() => setActiveThumb(i)} aria-label={`Image ${i + 1}`}><img src={src} alt={`Thumb ${i + 1}`} /></button>)}</div>}
        </div>
