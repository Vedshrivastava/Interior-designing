...
const CheckShield = () => (
  <svg viewBox="0 0 20 20" fill="none" width="12" height="12" style={{ flexShrink: 0 }}>
    <path d="M10 2L3 5v5c0 4.418 2.986 7.46 7 8 4.014-.54 7-3.582 7-8V5l-7-3z" fill="#34A853" opacity="0.15"/>
    <path d="M10 2L3 5v5c0 4.418 2.986 7.46 7 8 4.014-.54 7-3.582 7-8V5l-7-3z" stroke="#34A853" strokeWidth="1.5"/>
    <path d="M7 10l2 2 4-4" stroke="#34A853" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
    ...
    <div className="tr-page">
      ...
      {active && (
        <div className="t-modal-backdrop" onClick={() => setActive(null)}>
          <div className={`t-modal${active.isGoogle ? ' tr-modal-google' : ' t-modal--light'}`} onClick={e => e.stopPropagation()}>
            <button className="t-modal-close" onClick={() => setActive(null)} aria-label="Close"><IconXMark /></button>

            {active.isGoogle && (
              <div className="tr-modal-google-header">
                <div className="tr-modal-google-brand">
                  <GoogleSVG size={20} />
                  <span>Google Review</span>
                </div>
                <div className="tr-modal-verified">
                  <CheckShield /> Verified
                </div>
              </div>
            )}

            <div className="t-modal-stars">{Array.from({ length: active.rating }).map((_, i) => <IconStarFilled key={i} />)}</div>
            <p className={`t-modal-text${active.isGoogle ? ' tr-google-text' : ''}`}>{active.text}</p>
            <div className="t-modal-author">
              ...
              <div className="t-modal-author-info">
                ...
                <span>{active.isGoogle ? active.time : active.location}</span>
              </div>
            </div>

            {active.isGoogle && (
              <a
                href={active.reviewsUrl || googleUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="tr-verify-link"
                onClick={e => e.stopPropagation()}
              >
                <GoogleSVG size={14} />
                Verify this review on Google
                <IconArrowRight />
              </a>
            )}
          </div>
      ...
      <section className="tr-header">
        ...
        {googleReviews.length > 0 && googleUrl && (
          <a href={googleUrl} target="_blank" rel="noopener noreferrer" className="tr-google-link">
            <GoogleSVG size={16} /> See all our Google reviews
          </a>
        )}
      </section>

      {/* ── Google reviews strip (if any) ── */}
      {googleReviews.length > 0 && (
        <section className="tr-google-section">
          <div className="tr-google-section-head">
            <div className="tr-google-section-label">
              <GoogleSVG size={18} />
              <span>Google Verified Reviews</span>
              <div className="tr-verified-badge"><CheckShield /> Verified</div>
            </div>
            <p className="tr-google-section-sub">These reviews are pulled directly from our Google Business Profile and cannot be edited or removed by us.</p>
          </div>
          <div className="tr-grid tr-grid--google">
            {googleReviews.map((t, i) => (
              <div key={`g${i}`} className="tr-card tr-card--google" onClick={() => setActive(t)} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && setActive(t)}>
                <div className="tr-gcard-header">
                  <div className="tr-gcard-brand">
                    <GoogleSVG size={18} />
                    <div className="tr-gcard-verified"><CheckShield /><span>Verified</span></div>
                  </div>
                  <div className="tr-card-stars">{Array.from({ length: t.rating }).map((_, s) => <IconStarFilled key={s} />)}</div>
                </div>

                <p className="tr-card-text">{truncate(t.text)}</p>

                <div className="tr-card-author">
                  <div className="tr-card-avatar tr-gcard-avatar">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <a
                  href={t.reviewsUrl || googleUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="tr-gcard-verify"
                  onClick={e => e.stopPropagation()}
                >
                  <GoogleSVG size={12} /> Verify on Google <IconArrowRight />
                </a>
              </div>
            ))}
          </div>
      ...
      <section className="tr-grid-section">
        {googleReviews.length > 0 && (
          <p className="tr-section-label"><IconQuoteLeft /> Our Client Stories</p>
        )}
        {loading ? (
          <div className="tr-loading">Loading reviews…</div>
        ) : (
          <div className="tr-grid">
            {adminReviews.map((t, i) => (
              <div key={t._id || i} className="tr-card" onClick={() => setActive(t)} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && setActive(t)}>
                ...
                <div className="tr-card-author">
                  <div className="tr-card-avatar">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    {t.image ? <img src={t.image.src || t.image} alt={t.name} /> : t.name.charAt(0)}
                  </div>
                  <div className="tr-card-info">
                    <strong>{t.name}</strong>
